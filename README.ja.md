...
5. (任意) 同じ内容をブラウザで見ることもできます:

   ```bash
   claude-spillway dashboard          # http://127.0.0.1:8787/_spillway/ を開く
   ```

   ページはプロキシ自身が配信するため、別のプロセスを動かす必要はありません。
   両バックエンドの残量・リセットまでのカウントダウン・切替閾値を並べて表示し、
   自動で更新されます。プロキシが応答しなくなると即座に赤くなりますが、これは
   早く気づく価値のある状態です。`ANTHROPIC_BASE_URL`をここに向けている間は、
   Claude CodeからAnthropicへも到達できなくなるためです。
