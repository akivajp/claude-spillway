...
5. (任意) 同じ内容は <http://127.0.0.1:8787/_spillway/> でブラウザからも
   見られます。`serve`の起動時と`monitor`の画面にURLが表示されるため、
   覚えておく必要はありません。

   ページはプロキシ自身が配信するため、別のプロセスを動かす必要はなく、
   ネットワークから何も読み込みません。両バックエンドの残量・リセットまでの
   カウントダウン・切替閾値を並べて表示し、自動で更新されます。プロキシが
   応答しなくなると即座に赤くなりますが、これは早く気づく価値のある状態です。
   `ANTHROPIC_BASE_URL`をここに向けている間は、Claude CodeからAnthropicへも
   到達できなくなるためです。
